    ...
    public class AircraftLandingGear : MonoBehaviour {
        ...
        private void Update() {
            if (SceneManager.GetActiveScene().buildIndex == 1) {
                if (m_State == GearState.Lowered && m_Plane.Altitude > raiseAtAltitude && m_Rigidbody.velocity.y > 0) m_State = GearState.Raised;
                if (m_State == GearState.Raised  && m_Plane.Altitude < lowerAtAltitude && m_Rigidbody.velocity.y < 0) m_State = GearState.Lowered;
